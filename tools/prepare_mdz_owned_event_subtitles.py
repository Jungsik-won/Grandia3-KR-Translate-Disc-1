#!/usr/bin/env python3
"""Prepare a common SLPM renderer plus subtitle packages owned by each MDZ.

This tool deliberately stops before rebuilding an ISO.  It emits one patched
SLPM, one candidate MDZ per resource, a replacement plan, and a verification
report.  Subtitle glyphs, cue timing, and per-resource sample lookup tables
live in the owning MDZ; SLPM contains only the reusable locator/renderer.
"""
...
def build_packages(manifest: Path) -> dict[str, object]:
...
def patch_slpm(source: bytes, output: Path, packages: dict[str, object]) -> dict[str, object]:
    ...
    if source[data_off:data_off + atlas.DATA_CAVE_CAPACITY] != bytes(
            atlas.DATA_CAVE_CAPACITY):
        raise ValueError("SLPM common-data cave is not clean")
    common_data = bytes(packages["common_data"])
    ...
    return {
        ...
        "role": "COMMON_RENDERER_AND_MDZ_LOCATOR_ONLY",
        "subtitle_payload_bytes": 0,
...
def main() -> int:
    ...
    packages = build_packages(manifest)
    ...
    report = {
        "schema_version": 1,
        "architecture": "MDZ_OWNS_SUBTITLE_DATA_SLPM_OWNS_COMMON_RENDERER",
